...
export default function MealExercisePlanScreen({ navigation, route }: MealExercisePlanScreenProps) {
  ...
  const updateProgress = () => {
    ...
    if (dayData.exercise && dayData.exercise.exercises) {
      const completedExercisesCount = dayData.exercise.exercises.filter((ex: Exercise) => ex.isDone).length;
  ...
  const toggleMealStatus = async (mealType: 'breakfast' | 'lunch' | 'dinner') => {
    ...
    try {
      ...
      const response = await fetch(`https://api-fitemeal.vercel.app/api/meal-exercise-plans/${userId}/meals/${mealType}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
  ...
  const toggleExerciseStatus = async (exerciseIndex: number) => {
    ...
    try {
      ...
      const response = await fetch(`https://api-fitemeal.vercel.app/api/meal-exercise-plans/${userId}/exercises/${exerciseIndex}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
  ...
  const exerciseProgressPercentage = totalExercises > 0 ? (completedExercises / totalExercises) * 100 : 0;
        ...
        <View style={styles.progressContainer}>
          ...
          <View style={styles.progressCard}>
            <View style={styles.progressHeader}>
              ...
              <Text style={styles.progressCount}>{completedMeals}/{totalMeals}</Text>
            </View>
          ...
          <View style={styles.progressCard}>
            <View style={styles.progressHeader}>
              ...
              <Text style={styles.progressCount}>{completedExercises}/{totalExercises}</Text>
            </View>
            <View style={styles.progressBarContainer}>
              <View style={styles.progressBarBackground}>
                <View 
          ...
          {dayData && dayData.exercise && (
            <View style={styles.exerciseContainer}>
              <View style={styles.exerciseOverview}>
                ...
                <Text style={styles.exerciseCalories}>{dayData.exercise.caloriesBurned} calories</Text>
              </View>
